...
export function AuthenticationForm(props: PaperProps) {
  ...
  const form = useForm({
    ...
    const {usernamers} = useContext(AuthContext);
    ...
    const handleLogin = async () => {
      try {
        ...
        if (response.status !== 200) {
          ...
          const test = await fetchDecodedAccessToken();
          console.log('User:', usernamers);
    ...
    const handleRegister = async () => {
      try {
        toggle(); 
        const response = await axios.post('register/', { username, password, first_name, last_name, email,role });
  ...
  return (
    ...
    {/* /*{ <Group grow mb="md" mt="md">
      ...
      onOptionSubmit={(val) => {
        setValue(val);
        setRole(removeEmojis(val).toLowerCase());
    ...
    >
      ...
      <Combobox.Dropdown>
        ...
        <Group justify="space-between" mt="xl">
          ...
          <Button type="submit" radius="xl" onClick={type === 'login' ? handleLogin : handleRegister}>
            {upperFirst(type)}
          </Button>
        </Group>
      </form>
