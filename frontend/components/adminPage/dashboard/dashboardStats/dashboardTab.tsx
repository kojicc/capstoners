...
export function StatsRing() {
  ...
  const { data:MostReservedProduct, error:MostReservedProductError } = useSWR('most-reserved-products/', fetcher, { refreshInterval: 1000 });
  ...
  const { data:PendingOrders, error:PendingOrdersError } = useSWR('pending-orders/', fetcher, { refreshInterval: 1000 });
  ...
  const { data:TotalUsers, error:TotalUsersError } = useSWR('total-users/', fetcher, { refreshInterval: 1000 });
  ...
  return <SimpleGrid pb={20} cols={{ base: 1, sm: 3 }}>{stats}</SimpleGrid>;
}