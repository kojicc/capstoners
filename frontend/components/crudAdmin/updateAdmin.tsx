...
import classes from '../modules.css/TableSort.module.css';
import { notifications } from '@mantine/notifications';
import moment from 'moment-timezone';
import styles from '../modules.css/TableSort.module.css';
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.status === 200) {
        if(categoryID){
          const filteredProducts = response.data.products.filter((product: { categoryId: string; }) => product.categoryId === categoryID);
...
<>
  <Table className={styles.table} horizontalSpacing="xl" verticalSpacing="xs">
    <thead>
      ...
      <Th sorted={sortBy === 'productId'} reversed={reverseSortDirection} onSort={() => handleSort('productId')}>
              Product ID
            </Th>

            <Th sorted={sortBy === 'name'} reversed={reverseSortDirection} onSort={() => handleSort('name')}>
              Product Name
            </Th>

            <Th sorted={sortBy === 'description'} reversed={reverseSortDirection} onSort={() => handleSort('description')}>
              Product Description
            </Th>

            <Th sorted={sortBy === 'price'} reversed={reverseSortDirection} onSort={() => handleSort('price')}>
              Product Price
            </Th>

            <Th sorted={sortBy === 'quantity'} reversed={reverseSortDirection} onSort={() => handleSort('quantity')}>
              Product Quantity
            </Th>
            
            <Th sorted={sortBy === 'image'} reversed={reverseSortDirection} onSort={() => handleSort('image')}>
              Product Image
              </Th>
      ...
      {paginatedData.map((products) => {
        ...
        return (
          <tr key={products.productId} className={styles.tr} id={products.productId}>
            ...
            <td className={styles.td}> <img style={{ margin: 'auto', borderRadius: 'md', height: 100, width: 100 }} src={`http://localhost:8000${products.image}`} alt={products.name} /></td>
            <td className={styles.td}>
              <Group gap="xs">
                <ActionIcon onClick={() => { setSelectedProducts(products); setEditModalOpened(true); }}>
                  <IconEdit />
                </ActionIcon>
                <ActionIcon color="red" onClick={() => { setSelectedProducts(products); setDeleteModalOpened(true); }}>
                  <IconTrash />
                </ActionIcon>
              </Group>
            </td>
...
<Stack>
  ...
  disabled
    ...
    onChange={(event) =>
      setSelectedProducts((prev) => ({ ...prev, productId: event.currentTarget.value } as Product))
    }
  />
  ...
  <TextInput
    ...
    onChange={(event) =>
      setSelectedProducts((prev) => ({ ...prev, name: event.currentTarget.value } as Product))
    }
  />
  ...
  <TextInput
    ...
    onChange={(event) =>
      setSelectedProducts((prev) => ({ ...prev, description: event.currentTarget.value } as Product))
    }
  />
  ...
  disabled
    ...
    onChange={(value) => setSelectedProducts((prev) => ({ ...prev, category: value! } as Product))}
    />
